...
only way to know it agrees with the regulator is to run the regulator's own
tool against our output. That is what `scripts/kosit-check.sh` does.
...
schematron parity — we implement roughly 30 of the ~180 rules that apply to a
UBL invoice under XRechnung. A document our engine accepts can still be rejected
